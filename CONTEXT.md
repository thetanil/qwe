...
**Trust boundary**:
The workflow directory and the inventory are **trusted input**, in the way a `Makefile` or a `.github/` directory is. Project plugin code runs with the operator's full privileges on the operator host, and a workflow can already run any command through `run:`, so qwe does not try to contain either. Running a workflow directory someone else wrote is running their code as you; read it first. `qwe validate` is the one exception, and only because it was made one: it executes nothing (below, and ADR-0013). The inventory is trusted the same way: it names the hosts qwe connects to and holds the secrets a job can read.

What `qwe validate` does with a project plugin: it reads the source, runs luacheck over it, checks that it compiles, and analyses its shape (`qwe.pluginshape`), all **without running any of it**. A plugin file may hold only declarations and a `return` of its module table: local constants, function literals, table constructors, `M.name = value` assignments and `require` of modules qwe provides. **Top-level code is refused**: a call, a loop, an `if`, a do-block, a rebind, a require of some other file, anything that would run when the file loads. `check` and `apply` must both be exported as functions (or `argv`, for a run-like plugin), and that is also decided from the source. So loading a validated plugin cannot change state; its work happens in `check` and `apply`, in a step's forked child during `qwe run`. Pinned by `tests/e2e/plugin_top_level_refused` and `src/kernel/lua/pluginshape_test.lua`. Running a workflow directory you did not write still runs its plugins as you. A load-time hook (an `init`) is not offered; it would be the only place setup could go. Why validation reads the source instead of loading it, and what that costs a plugin author: ADR-0013.
