...
- **Every master is opened before the first job starts**, one per distinct target the selected jobs use, and never while a job or step timer is armed. A job whose steps all run `on: local` never needs its target, so its target is not contacted. The parent blocks on a connect, so doing it inside the event loop makes one slow host delay every other job's timeout — measured at 10 s, against a step that asked for 1 s. Connecting up front keeps the timeout guarantee exact (§I.5) and costs about 306 ms per target, paid before any timer exists. A target that cannot be reached fails its own jobs with reason `unreachable`; the rest of the run proceeds.
...
- **The ControlPath is short and per run** (`<dir>/<qwe pid>.%C`), because Unix socket paths are limited to about 108 bytes, and test and sandbox temp directories are often longer than that. **`<dir>` is `$XDG_RUNTIME_DIR/qwe` when that variable names an existing directory** (the system made it per-user and 0700), and `/tmp/qwe-<uid>` otherwise. Either way the directory is checked before every use, not assumed: it must be a real directory (not a symlink), owned by this user, with no group or other permission bits, the same rule the key file is held to. A directory that fails is refused: the step fails with a message naming it and what is wrong, and qwe does not fall back to a less safe path.
...
1. The connection-pool plugin ensures a **master connection** is running for a host: `ssh -M -N -o ControlPersist=<ttl> …`, which holds a **control socket** (a Unix domain socket managed by OpenSSH).
   **The ttl is 120 s.** It is a backstop, not the mechanism: a normal end still closes every master at once with `ssh -O exit`. It exists for the run that never gets to (SIGKILL, a crash), whose master would otherwise hold its connection and socket until reboot. 120 s is long enough that a job queued behind `max-sessions`, or a gap between two steps, keeps the connection rather than paying a new handshake (about 306 ms), and short enough that an abandoned master is gone in two minutes. A run also removes, once, the sockets in its socket directory that a killed run left: those of a qwe that is no longer running and whose master no longer answers. `QWE_TEST_MASTER_TTL` (seconds) overrides the ttl for tests.
...
- **Every master is opened before the first job starts**, for the targets the selected jobs actually use, so that a **timeout fires within 100 ms** of its deadline whatever else the run is doing (I.2, I.5). The single exception is a mid-run reconnect, ≤3 s, on an error path. A target that cannot be connected to fails its own jobs with **`unreachable`**, never `engine-error`, and the rest of the run proceeds. There is **no flag to disable pre-connect**: an option that quietly downgrades a timing guarantee is worse to own than the three lines it saves.
- **`ControlPersist=120` is a backstop, not the mechanism** (I.3). A normal end closes every master at once; the ttl bounds the master left by a run that was SIGKILLed, which would otherwise hold its connection until reboot. A run also sweeps the sockets such runs left behind.
- **The ControlPath directory is verified before every use** (I.2): a real directory, owned by this user, no group or other permission bits — the same rule the key file is held to. `$XDG_RUNTIME_DIR/qwe` is preferred, `/tmp/qwe-<uid>` is the fallback, and a directory that fails the check is refused rather than fallen back from.
...
- **The parent creates a `run:` step's `$QWE_OUTPUT` file `0600`** before the fork, and `.qwe/runs/<id>/` `0700` (II.9), so a secret step output is never world-readable for the life of the step that wrote it.
