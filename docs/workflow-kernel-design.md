...
`qwe validate` runs three checks on every project plugin, **none of which executes plugin code**: its schemas against the metaschema, its **declared contract** (a `kind` of `check-apply` or `argv`, a `with:` schema, and outputs with `secret` flags), and that it compiles, and **luacheck**. Built-in plugins pass the same checks as Bazel tests. All plugin code runs with strict globals. A plugin is `plugin.lua` plus `schema.json` (`{"kind": …, "with": …, "outputs": …}`); kind `check-apply` means the module exports `check` and `apply`, kind `argv` a run-like one that exports `argv`. Whether the module exports them is checked when a step first uses it, because that takes loading it (a failure is `plugin-error`). Only luacheck's API is embedded (no argparse, no lfs). A `uses:` step whose plugin raises an error fails with reason `plugin-error`.
...
- **Timeouts** (`timeout-seconds:` on a job or a step) are in **seconds**, decimals allowed (`0.3` is 300 ms); there is no other unit and `timeout-minutes` is rejected with the equivalent value in the message. The value must be above 0 and at most **604800** (seven days), a validation error otherwise. The engine converts to whole milliseconds and rounds any positive value **up** to at least 1 ms, so a requested timeout is never turned into `0`, which means "no limit". An absent `timeout-seconds` means no limit.
  - The preamble (`src/kernel/preamble.c`): a command starts as `sh -c <bootstrap> sh <script>`. Its stdin is one `NAME=value` line per variable (`\` and newline escaped), an empty line, then the command's own stdin. The bootstrap reads exactly that, exports it and `exec`s the script, so the rest of stdin is untouched. A `run:` step gets it from an anonymous file the child sets as stdin before it execs; `backend:run` prepends it to the stdin it passes.
