...
`qwe validate` runs three checks on every project plugin: its schemas against the metaschema, its **plugin shape** (a `with:` schema, outputs with `secret` flags, and a `plugin.lua` that is declarations and a `return` only, with **no top-level code** and `check` and `apply` (or `argv`, for a run-like plugin) exported as functions), and **luacheck**. Validation decides all of this from the source and runs none of it. Built-in plugins pass the same checks as Bazel tests. All plugin code runs with strict globals. A plugin is `plugin.lua` plus `schema.json` (`{"with": …, "outputs": …}`); a step plugin exports `check` and `apply`, a run-like one exports `argv`. Only luacheck's API is embedded (no argparse, no lfs). A `uses:` step whose plugin raises an error fails with reason `plugin-error`.
