...
int qwe_key_default_path(char *out, size_t n)
{
	...
	if (!home || !*home)
		return -1;
	return qwe_fmt(out, n, "%s/.config/qwe/secret", home);
}
...
static int resolve(const char *path, char *buf, size_t n, const char **out, char *err, size_t err_size)
{
	...
	if (qwe_key_default_path(buf, n) < 0) {
		...
		qwe_msg(err, err_size, home && *home ? "no key file: HOME is too long" : "no key file: HOME is not set");
		return -1;
	}
	*out = buf;
	return 0;
...
int qwe_private_check(const struct stat *st, const char *what, const char *path, char *err, size_t err_size)
{
	if (st->st_uid != geteuid()) {
		qwe_msg(err, err_size, "%s %s is owned by uid %u, not by you (uid %u)", what, path,
			 (unsigned)st->st_uid, (unsigned)geteuid());
		return -1;
	}
	if (st->st_mode & 077) {
		qwe_msg(err, err_size,
...
int qwe_key_load(const char *path, uint8_t key[QWE_KEY_BYTES], char *err, size_t err_size)
{
	...
	if (fstat(fd, &st) < 0 || !S_ISREG(st.st_mode)) {
		qwe_msg(err, err_size, "the key file %s is not a regular file", path);
		close(fd);
		return -1;
	}
	...
	if (got == QWE_KEY_BYTES) {
		...
		if (got <= 0 || sodium_base642bin(key, QWE_KEY_BYTES, raw, (size_t)got, NULL, &bin_len, NULL,
			...
			qwe_msg(err, err_size, "the key file %s is neither 32 raw bytes nor base64 of 32 bytes", path);
...
int qwe_key_generate(const char *path, char *err, size_t err_size)
{
	...
	if (qwe_secrets_init() < 0) {
		qwe_msg(err, err_size, "cannot initialize libsodium");
		return -1;
	}
	if (qwe_fmt(dir, sizeof dir, "%s", path) < 0) {
		qwe_msg(err, err_size, "the key file path is too long (over %lu bytes): %.64s...",
			(unsigned long)(sizeof dir - 1), path);
		return -1;
	}
	slash = strrchr(dir, '/');
	if (slash && slash != dir) {
